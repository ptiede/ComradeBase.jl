...
function StokesIntensityMap(
        I::IntensityMap{T}, Q::IntensityMap{T}, U::IntensityMap{T},
        V::IntensityMap{T}
    ) where {T}
    @assert check_grid(I, Q, U, V) "Intensity grids are not the same across the 4 stokes parameters"
    polimg = StructArray{StokesParams{T}}(;
        I = baseimage(I), Q = baseimage(Q), U = baseimage(U),
        V = baseimage(V)
    )
...
@inline intensity_point(s::SingleStokes{M, S}, x, y) where {M, S} = getproperty(
    intensity_point(
        s.model,
        x,
        y
...
function StokesIntensityMap(
        ...
        dims::NamedTuple{Na, <:NTuple{N, Any}}
    ) where {T, N, Na}
...
function _summary(io, x::StokesIntensityMap{T, N, Na}) where {T, N, Na}
    ...
    for d in 1:ndims(x)
        ...
        printstyled(
            io, length(axiskeys(x, d)), "-element ",
